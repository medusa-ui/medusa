...
public class MedusaOnChangeTest extends MedusaTagTest {
    ...
    void basicRenderTest() {
        ...
        Assertions.assertTrue(template.contains("onchange=\"_M.doAction(event, '__FRAGMENT__', `action('${_M.findValueById(`#cnt`, `value`)}')`)\""), "Medusa tag should be rendered with replacement JS");
    }
    ...
    void selectRenderTest() {
        ...
        Assertions.assertFalse(template.contains("th:text"), "Thymeleaf tags should be rendered");
        Assertions.assertFalse(template.contains("m:change"), "Medusa tags should be rendered");
        Assertions.assertTrue(template.contains("onchange=\"_M.doAction(event, '__FRAGMENT__', `search('${_M.findValueById(`#search-select`, `value`)}')`)\""), "Medusa tag should be rendered with replacement JS");
    }
